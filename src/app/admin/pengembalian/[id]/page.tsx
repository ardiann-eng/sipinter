...
export default async function ReturnDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const request = await db.borrowingRequest.findFirst({
    where: { id, status: { in: [BorrowingStatus.WAITING_RETURN_VERIFICATION, BorrowingStatus.RETURN_PROBLEM, BorrowingStatus.COMPLETED] } },
    include: {
  ...
  const completed = request.status === BorrowingStatus.COMPLETED;
  ...
  return <>
    ...
    <Panel title="Surat persetujuan"><ApprovalLetter requestId={request.id} signed={Boolean(request.approvedAt)} /></Panel>
    <div className={s.compare}>
      ...
      <Panel title="Bukti pengembalian" description={`Dikirim ${formatDateTime(submission.submittedAt ?? submission.createdAt)}`}><dl className="detail-grid"><div className="detail-item"><dt>Kondisi dilaporkan</dt><dd>{submission.submittedCondition}</dd></div><div className="detail-item"><dt>Kelengkapan</dt><dd>{submission.completenessStatus}</dd></div><div className="detail-item"><dt>Catatan peminjam</dt><dd>{submission.notes || "Tidak ada catatan"}</dd></div></dl><div className="return-evidence" style={{ marginTop: 16 }}>{submission.photos.length ? submission.photos.map((photo) => <a key={photo.id} href={`/api/uploads/${encodeURIComponent(photo.storageKey ?? photo.fileUrl)}`} target="_blank" rel="noreferrer"><span className="return-evidence__preview" style={{ backgroundImage: `url(/api/uploads/${encodeURIComponent(photo.storageKey ?? photo.fileUrl)})` }} /><span><Camera size={16} aria-hidden="true" /> {photo.originalName || "Buka foto bukti"}</span></a>) : <p>Tidak ada foto bukti yang ditambahkan.</p>}</div></Panel>
    </div>
    {!completed && <div className={s.grid7030}>
