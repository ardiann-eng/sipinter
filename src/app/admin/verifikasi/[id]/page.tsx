...
export default async function VerificationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><AdminHeader title="Pemeriksaan permohonan" description={request.registrationNumber} actions={<Status value={request.status} />} /><div className={s.grid7030}>
    <div className={s.stack}>
      ...
      <Panel title="Kendaraan yang dimohon" flush><TableContainer><Table><TableHead><TableRow><TableHeader>Kendaraan</TableHeader><TableHeader>Jumlah</TableHeader><TableHeader>Ketersediaan</TableHeader></TableRow></TableHead><TableBody>{request.items.map((entry) => <TableRow key={entry.id}><TableCell><div className={s.vehicleCell}>{entry.item.mainPhoto && <Image className={s.vehicleThumb} src={entry.item.mainPhoto} alt="" width={72} height={48} />}<div><strong>{entry.item.name}</strong><small>{entry.item.itemCode}</small></div></div></TableCell><TableCell>{entry.quantity} {entry.item.unit}</TableCell><TableCell>{entry.item.availableQuantity} {entry.item.unit}</TableCell></TableRow>)}</TableBody></Table></TableContainer></Panel>
      <Panel title="Dokumen pendukung" description="Dokumen tersimpan pada penyimpanan privat."><div className={s.stack}><ApprovalLetter requestId={request.id} signed={Boolean(request.approvedAt)} /><DocumentPlaceholder name="Kartu Tanda Penduduk" type="document" description={request.ktpFile ? "Dokumen tersedia" : "Dokumen tidak tersedia"} href={request.ktpFile ? `/api/uploads/${encodeURIComponent(request.ktpFile)}` : undefined} /><DocumentPlaceholder name="Surat tugas / dokumen pendukung" type="document" description={request.approvalLetterFile ? "Dokumen tersedia" : "Dokumen tidak tersedia"} href={request.approvalLetterFile ? `/api/uploads/${encodeURIComponent(request.approvalLetterFile)}` : undefined} /></div></Panel>
    </div>
    <aside className={s.stack}><Panel title="Checklist verifikator" description="Pastikan seluruh butir telah diperiksa sebelum meneruskan."><div className={s.checklist}>{["Identitas dan NIP pemohon sesuai", "Surat permohonan sah dan terbaca", "Tujuan penggunaan bersifat kedinasan", "Jadwal kendaraan tidak bertabrakan", "Kapasitas sesuai jumlah penumpang", "Kondisi kendaraan telah dikonfirmasi"].map((label) => <label className={s.check} key={label}><input type="checkbox" /> <span>{label}</span></label>)}</div></Panel><Panel title="Catatan keputusan">{actionable ? <VerificationActions requestId={request.id} /> : <p>Permohonan sudah diproses pada {formatDateTime(request.updatedAt)} WITA.</p>}</Panel></aside>
  </div></>;
}
