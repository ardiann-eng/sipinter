...
export default async function AuditPage({ searchParams }: { searchParams: Promise<{ q?: string; module?: string }> }) {
  ...
  return <>
    ...
    <form className="filter-bar" method="get"><div className="filter-bar__search"><input name="q" defaultValue={q} aria-label="Cari audit" placeholder="Cari pelaku, objek, atau ID" /></div><select className="select" name="module" defaultValue={moduleFilter ?? ""} aria-label="Modul"><option value="">Semua modul</option><option value="AUTH">Autentikasi</option><option value="BORROWING">Peminjaman</option><option value="RETURN">Pengembalian</option><option value="INVENTORY">Kendaraan</option></select><button className="button button--outline" type="submit">Terapkan</button></form>
