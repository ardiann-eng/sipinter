...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const verification = await verifyReturn(id, actor, { ...parsed.data, issueType: parsed.data.issueType ?? undefined, issueDescription: parsed.data.issueDescription ?? undefined, followUpRecommendation: parsed.data.followUpRecommendation ?? undefined, resolutionNote: parsed.data.resolutionNote ?? undefined, context: { ipAddress: request.headers.get("x-forwarded-for"), userAgent: request.headers.get("user-agent") } });
