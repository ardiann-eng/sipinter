...
export default async function BorrowingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.grid}>
      ...
      <Panel className={styles.span4} title="Dokumen pengajuan"><div className={styles.available}>{record.ktpFile && <a className={styles.availableItem} href={`/api/uploads/${encodeURIComponent(record.ktpFile)}`} target="_blank" rel="noreferrer"><span className={styles.itemIcon}><FileText size={18} aria-hidden="true" /></span><div><strong>Kartu Tanda Penduduk</strong><small>Buka dokumen</small></div></a>}{record.approvalLetterFile && <a className={styles.availableItem} href={`/api/uploads/${encodeURIComponent(record.approvalLetterFile)}`} target="_blank" rel="noreferrer"><span className={styles.itemIcon}><FileText size={18} aria-hidden="true" /></span><div><strong>Surat tugas</strong><small>Buka dokumen</small></div></a>}</div></Panel>
    </div>
  </div>;
}
