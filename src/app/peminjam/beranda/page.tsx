...
export default async function BorrowerHomePage() {
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.grid}>
      ...
      <section className={`${styles.facilityList} ${styles.span12}`}><header><div><p className={styles.sectionEyebrow}>KATALOG KENDARAAN</p><h2>Kendaraan tersedia</h2><span>Pilih kapasitas yang sesuai dengan jumlah penumpang.</span></div><Link className={`${styles.linkButton} ${styles.linkOutline}`} href="/peminjam/ajukan">Pilih kendaraan <ArrowRight size={16} aria-hidden="true" /></Link></header><div className={styles.available}>{availableItems.map((item) => <div className={styles.facilityItem} key={item.id}>{item.imageUrl ? <span className={styles.facilityImage}><Image src={item.imageUrl} alt="" fill sizes="64px" /></span> : <span className={styles.itemIcon}><Box size={18} aria-hidden="true" /></span>}<div><strong>{item.name}</strong><small>{item.registrationNumber} · {item.code}</small></div><Badge tone="success">{item.availableStock} {item.unit}</Badge></div>)}</div></section>
    </div>
  </div>;
}
