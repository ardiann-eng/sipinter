...
export function DecisionPanel({ requestId, disabled = false }: { requestId: string; disabled?: boolean }) {
  ...
  return <>
    <aside className="decision-panel">
      ...
      <p>Pastikan identitas, tujuan kedinasan, periode, kendaraan, kapasitas, dan catatan verifikasi telah ditelaah.</p>
    ...
    <Modal open={decision !== null} onClose={() => { setDecision(null); setError(""); }} title={decision === "approve" ? "Konfirmasi persetujuan" : "Tolak permohonan"} description={decision === "approve" ? "Keputusan akan meneruskan permohonan ke proses penyiapan dan penyerahan kendaraan." : "Penolakan menghentikan proses permohonan ini."} footer={<><Button variant="ghost" onClick={() => setDecision(null)}>Batal</Button><Button variant={decision === "approve" ? "secondary" : "danger"} onClick={submit}>{decision === "approve" ? "Ya, setujui" : "Catat penolakan"}</Button></>}>
