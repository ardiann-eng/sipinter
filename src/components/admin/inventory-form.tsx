...
export function InventoryForm({ item }: { item?: InventoryItem }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !result.id) throw new Error(result.error ?? "Data barang belum dapat disimpan.");
      router.push(`/admin/barang/${result.id}`);
      router.refresh();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Data barang belum dapat disimpan.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return <form className={s.stack} onSubmit={submit} aria-busy={busy}>
    ...
    <Panel title="Identitas barang" description="Kolom bertanda bintang wajib diisi.">
    ...
    <div className={s.stickyActions}><Link className={s.linkButton} href="/admin/barang">Batal</Link><Button type="submit" loading={busy} aria-describedby={error ? "inventory-form-error" : undefined}>{busy ? "Menyimpan..." : "Simpan data barang"}</Button></div>
  </form>;
}

export function InventoryExportButton({ items }: { items: InventoryItem[] }) {
  function download() {
    const rows = [["Kode", "Identitas", "Nama", "Kategori", "Tersedia", "Total", "Kondisi", "Status", "Lokasi"], ...items.map((item) => [item.code, item.registrationNumber, item.name, item.category, item.availableStock, item.totalStock, item.condition, item.status, item.location])];
